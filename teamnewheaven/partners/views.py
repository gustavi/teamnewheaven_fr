...
def partners_list(request):
    """
    Displays the list of partners and friends.
    """
    partners = Partner.objects.filter(is_partner=True).order_by('?')
    friends = Partner.objects.filter(is_partner=False).order_by('?')
    return render(request, 'partners/partners-list.html', locals())
